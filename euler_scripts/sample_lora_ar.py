...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Directory containing base AutoBrep checkpoints",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Output directory for generated STEP files",
    )
    parser.add_argument(
        "--num_samples", type=int, default=10, help="Number of CAD samples to generate"
    )
    parser.add_argument(
        "--temperature", type=float, default=0.8, help="Sampling temperature"
    )
    parser.add_argument(
        "--threshold", type=float, default=0.95, help="Top-p sampling threshold"
    )
    parser.add_argument(
        "--complexity",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of samples to generate concurrently (e.g. 10 for 24GB GPUs)",
    )
    parser.add_argument(
...
def main():
    ...
    with torch.no_grad():
        ...
        for batch_idx in range(num_batches):
            current_batch_size = min(
                args.batch_size, args.num_samples - batch_idx * args.batch_size
            )
            print(
                f"Generating batch {batch_idx + 1}/{num_batches} ({current_batch_size} samples)..."
            )
    ...
    total = len(batch_cad_data)
    print(
        f"\n📊 Success rate: {success_count}/{total} ({100 * success_count / max(total, 1):.1f}%)"
    )
    print(f"🎉 Inference and Export Complete. Files saved to {output_dir}")
