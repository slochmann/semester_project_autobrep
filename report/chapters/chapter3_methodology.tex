...
\begin{figure}[h!]
	...
	\caption{Two examples of preprocessed point cloud solids from the generic cylinders with holes}
...
\textbf{B-Rep Reconstruction and Validation:}

Token sequences are decoded into geometric primitives (surfaces, curves) and topological relationships via the FSQ-VAE decoders. The reconstructed B-Rep geometry is then validated and assembled into watertight solids using the AutoBrepBuilder with the following numerical tolerances:

\begin{table}[h!]
	\centering
	\caption{B-Rep Reconstruction Tolerances}
...
A B-Rep is considered valid if it passes the solid modeling kernel's validation checks: all edges are shared by exactly 2 faces (no dangling edges), the topology forms a closed manifold (watertight), and geometric primitives lie within valid numerical precision.
